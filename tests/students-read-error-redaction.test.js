...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'classroom-students-read-error-redaction-test-'));
...
test('Students Read Error Redaction Tests', async (t) => {
    ...
    const getStudentsRoutes = app._router.stack.filter(
        layer => layer.route && layer.route.path === '/api/students' && layer.route.methods.get
    );

    await t.test('Exactly one GET /api/students route exists', () => {
        assert.strictEqual(getStudentsRoutes.length, 1);
    });

    const getHandler = getStudentsRoutes[0].route.stack[getStudentsRoutes[0].route.stack.length - 1].handle;
    ...
    await t.test('Successful response path', async () => {
        ...
        const representativeRows = [
            { id: 1, name: 'Ada Yılmaz', photo: null, gender: 'F' },
            { id: 2, name: 'Mert Demir', photo: '/uploads/mert.jpg', gender: 'M' }
        ];
        ...
        assert.strictEqual(capturedSql, 'SELECT * FROM students', 'SQL is exactly expected');
        ...
        assert.deepStrictEqual(res.body, representativeRows, 'Response body exactly equals the representative rows');
    ...
    await t.test('Database error path', async () => {
        ...
        const secretMarker = 'SENSITIVE_STUDENT_LIST_DB_DETAIL_6a31f8';
        ...
        assert.deepStrictEqual(res.body, { error: 'Öğrenciler alınırken hata oluştu' }, 'Response body is exactly expected');
        ...
        assert.strictEqual(loggedMessage, 'Error fetching students', 'logger message is exactly expected');
        ...
        assert.strictEqual(capturedSql, 'SELECT * FROM students', 'The captured SQL is exactly expected');
