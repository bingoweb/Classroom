...
// Database Isolation
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'classroom-role-bounded-duplicate-redaction-'));
...
test('Role Bounded Duplicate Error Redaction Tests', async (t) => {
    t.after(async () => {
        try {
    ...
    for (const rType of roles) {
        await t.test(`Role ${rType}`, async (subT) => {
            ...
            // Duplicate-query error redaction tests
            await subT.test('Duplicate error - secret marker', async () => {
                ...
                assert.strictEqual(metadata.query, duplicateSqlPassedToDbGet);
                assert.strictEqual(metadata.query, actualDupSql); // identity
                assert.strictEqual(metadata.params, actualDupParams); // identity
                assert.deepStrictEqual(metadata.params, expectedDuplicateParams[rType]); // content
            ...
            await subT.test('Duplicate error - legacy message', async () => {
                const fakeError = new Error('Duplicate lookup error');
                ...
                assert.strictEqual(result.statusCode, 500);
                assert.deepStrictEqual(JSON.parse(result.body), { error: 'Rol atanırken hata oluştu' });
                assert.ok(!result.body.includes('Duplicate lookup error'));
