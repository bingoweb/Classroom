...
// Database Isolation
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'classroom-role-bounded-count-redaction-'));
...
test('Role Bounded Count Error Redaction Tests', async (t) => {
    t.after(async () => {
        try {
    ...
    for (const rType of roles) {
        await t.test(`Role ${rType}`, async (subT) => {
            ...
            await subT.test('Duplicate classification', async () => {
                let runSql, runParams;
                db.run = function (sql, params, cb) {
                    runSql = sql;
                    runParams = params;
            ...
            await subT.test('Missing-student classification', async () => {
                ...
                db.get = function (sql, params, cb) {
                    ...
                    if (sql.includes('SELECT 1 FROM students WHERE')) {
            ...
            // Count-query error redaction tests
            await subT.test('Count error - secret marker', async () => {
                ...
                assert.strictEqual(metadata.query, countSqlPassedToDbGet);
                assert.strictEqual(metadata.query, actualCountSql); // identity
                assert.strictEqual(metadata.params, actualCountParams); // identity
                assert.deepStrictEqual(metadata.params, expectedCountParams[rType]); // content
            ...
            await subT.test('Count error - legacy message', async () => {
                const fakeError = new Error('Classification error');
                ...
                assert.strictEqual(result.statusCode, 500);
                assert.deepStrictEqual(JSON.parse(result.body), { error: 'Rol atanırken hata oluştu' });
                assert.ok(!result.body.includes('Classification error'));
