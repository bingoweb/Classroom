...
#### Bir Sonraki Önerilen Görev
Şu an için yeni bir özellik önerisi bulunmamaktadır. Proje sadece bakım modundadır.

### 11. Guardrail Hardening v3

**Tarih:** 2026-07-13
**Başlangıç Hash (Remote HEAD):** `2c4186e358ad1c564d386a529d7ca341ceff2afa`
**Implementation Message:** `fix: close maintenance guardrail enforcement gaps`

#### Yapılan Değişiklikler ve Bakım Modu Politikası
- Proje artık resmi olarak **bakım moduna** (maintenance mode) girmiştir. Yeni özelliklerin eklenmesi varsayılan olarak devre dışı bırakılmıştır.
- Tüm projenin bakımını kontrol altında tutan, yeni özellikleri, izinsiz UI değişikliklerini (yeni route, buton, tablo) engelleyen zorunlu bir `maintenance-policy guardrail` sistemi kurulmuştur.
- Görev sözleşmeleri (`task-contract`) artık sıkı bir şekilde enforce edilmekte ve mixed-commit (örneğin sözleşme ile kod değişikliğinin aynı commit'te olması) yapılmasına izin verilmemektedir. Kurallar hem yerel olarak hem de CI aşamasında doğrulanır.
- Tüm ajanlar, kod eklemeden önce kapsam belirten `task-contract.json` bildirimini yapmalıdır.
...
6. **Admin Schedule Diagnostics:** 84
7. **Admin Schedule Draft Editor:** 121
8. **Admin Schedule Review Panel:** 65
9. **Agent Compliance Guardrails:** 15

Bu hardening çalışması sonucunda Agent Compliance Guardrails test takımı silinip 15 adet çok daha kapsamlı olan yepyeni davranışsal test ile değiştirilmiştir.

##### 1. Kalıcı yapısal test bildirimleri
Toplam core test sayısı 495 olmuştur.

#### Bir Sonraki Önerilen Görev
Bütün proje genelinde feature envanterinin çıkarılması ve gerekli proaktif bakım ihtiyaçlarının planlanması (Inventory Existing Features).
