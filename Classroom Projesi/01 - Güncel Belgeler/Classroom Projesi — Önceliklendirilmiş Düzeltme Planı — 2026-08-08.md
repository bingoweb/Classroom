...
4. Living table P3-1/P3-2/Faz-0 durumları 9 Ağustos'ta gerçek milestone geçmişiyle uzlaştırılırken yeni regression test ilk local GREEN'den sonra Linux CI'da path portability kusuru yakaladı. `5af27b6391fcfc3bc7cbf247d3a9af507cd53e28` için GitHub Actions `31329103380` Node 22/24 FAIL nedeni, macOS'ta çalışan decomposed `Güncel` parent path'in Git/Linux checkout'taki composed `Güncel` path ile byte-level eşleşmemesiydi. Test artık `Classroom Projesi/` altında living-plan basename'ini Unicode-normalized discovery ile buluyor; sentetik decomposed-folder regression'ı da eklendi. Fix commit `095b59de829ad220c1828f69b6022adb633a28c7`, GitHub Actions `31329411002`: Node 22 ve Node 24 **PASS**. Fresh docs contract **7/7**, full core **1484/1484 PASS**, SQLite lifecycle/lock taraması **NONE**.
...
Bilinen fresh-DB `error_logs` startup cleanup-order bug'ı değiştirilmedi. Korunan untracked devir belgeleri ve `docs/superpowers/` commit kapsamına alınmadı; P2-6 fiziksel 55\" 4K kabul kapısı açık kalır. **B4.4 kapanışında sıradaki alt dalga B4.5 — media preview idi; bu alt dalga aşağıdaki 21.0.15 kaydında tamamlanmıştır.**

### 21.0.15 9 Ağustos 2026 — P3-5B4.5 admin slides media preview extraction

B4 Slides beşinci alt dalgasında media prepare/reset, media-input change binding ve FileReader preview ownership'i `public/admin/js/slides.js` içine taşındı. Submit/create/update/delete ile slide settings bu değişikliğin dışında bırakıldı.

Uygulanan sınır:

- `public/admin/js/slides.js`: `prepareSlideMediaForm`, `resetSlideMediaForm`, `handleSlideMediaChange` ve ortak `renderExistingMediaPreview` ownership'i,
- edit/new/close form yolları media prepare/reset için artık shell callback kullanmıyor,
- `AdminSlides.init(...)` içinden `prepareMediaForm` / `resetMediaForm` injection'ları kaldırıldı,
- `slideMedia` change listener artık slide modülü init'i içinde bağlanıyor,
- existing image/GIF/video preview ve normalized admin media path davranışı korunuyor,
- 100 MB validation, file info, FileReader image/video preview ve empty-selection existing-media restore korunuyor,
- create/update/delete submit ile slide settings shell'de kalıyor.

Kanıtlar:

- pre-change focused baseline **19/19**,
- TDD RED: önceki slide kontratları geçerken yeni B4.5 ownership/behavior testleri doğru nedenle kırıldı; toplam **15 pass / 10 fail**,
- minimal GREEN `npm run test:admin-slide-module` **25/25**,
- geniş admin/slide/media/cache/notification/DOM-safety kapısı **139/139**,
- tam core **1435/1435**,
- system smoke PASS,
- `npm audit --omit=dev` **0 vulnerability**,
- syntax + `git diff --check` temiz,
- `public/admin/admin.js` **624 → 481 satır**, `public/admin/js/slides.js` **346 → 453 satır**,
- Chrome DevTools temp-DB gerçek UI: existing `default_boy.png` preview doğru; gerçek `change` event'i ile `browser.png` seçimi FileReader data URL + `Yeni Resim Önizlemesi` üretti; file input boşaltılınca existing preview geri geldi; horizontal overflow 0,
- Chrome network `slides.js` 200, `admin.js` 200, `/api/admin/slides` 200; console error/warn 0,
- Playwright `/admin/` → `admin-login.html?next=/admin/` auth redirect PASS; takip eden çağrıda bilinen tool-side `about:blank`,
...
Admin auth/session composition root içinde kalır. **A8 ayrı security-regression turunda değerlendirilmiş ve yeni auth route extraction yapılmaması bilinçli mimari karar olarak kapatılmıştır.** `/admin` guard'ın static serving'den önce, login/logout/session endpoint'lerinin ise static/upload setup'tan sonra kayıtlı olması ve aynı auth middleware'lerinin yedi domain route modülüne dependency olarak dağıtılması nedeniyle tek `registerAdminAuthRoutes()` modülü gerçek coupling azaltımı sağlamayacaktır.

### 21.1.1 9 Ağustos 2026 — P3-5A8 admin auth/session refactor değerlendirmesi

A8 bir “mutlaka dosya çıkarma” işi değil, güvenlik sınırını ayrı turda değerlendirme kapısı olarak ele alındı. Sonuç: mevcut composition root **korunacak**.

Kararı destekleyen teknik gerçekler:

- `backend/server.js` yaklaşık **420 satır**; auth'ın policy/store/cookie/limiter parçaları zaten `admin-auth-config.js`, `admin-session-store.js`, `admin-session-cookie.js`, `request-rate-limiter.js` içinde ayrılmış durumda.
- Server'da kalan auth parçaları CSRF secret/token, tek session-store instance'ı, `requireAdminSession`, write limiter instance'ı ve üç auth endpoint'inin composition/registration sorumluluklarıdır.
- `/admin` guard satırı static middleware'den önce; login/logout/session endpoint'leri static/upload setup'tan sonra kayıtlıdır. Tek-fazlı auth registration source-order'ı değiştirir; iki-fazlı factory/registration ise mevcut düzenden daha fazla indirection getirir.
- `requireAdminSession`, `requireCsrfToken`, `requireAdminWriteRateLimit` yedi domain route modülüne dependency injection ile verilmektedir; auth'ı başka dosyaya taşımak bu bağımlılığı ortadan kaldırmaz.

Security-regression kanıtı:

- auth/session/rate-limit/CORS/JSON focused grup **67/67 PASS**,
- Chrome gerçek UI login **200**, authenticated admin **200**, session `authenticated:true`, CSRF **64**, HttpOnly nedeniyle `document.cookie` boş,
- admin fetch wrapper'ı otomatik CSRF eklediği için backend kapısı ayrıca ham XHR ile test edildi: tokensız settings write **403**, doğru token ile **200**,
- UI logout **200**; logout sonrası session `false`, korumalı admin JSON isteği **401**,
- negatif 403/401 denemelerinden sonraki temiz login reload console error/warn/issue **0**,
